...
class BST{
	...
	public:
	    ...
	    void cancel_below(BSTNode<T>* ptr, double area){
	        if(ptr->key->getArea() <= area)
	            remove(ptr->key);
	    }
		
	    void remove_below(BSTNode<T>* ptr, double area){
	        if(!ptr)
	            return;

	        remove_below(ptr->right, area);
	        cancel_below(ptr, area);
	        remove_below(ptr->left, area);
	    }

	    void remove_below(double area){remove_below(root, area);}
