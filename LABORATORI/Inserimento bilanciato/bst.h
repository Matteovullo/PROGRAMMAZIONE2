...
class BST {
	...
	BSTNode<T>* remove(T key) {
		...
		void insert_destra(BSTNode<T>* ptr, T val){
			while(ptr->right){
				ptr=ptr->right;
			}
		...
		void insert_sinistra(BSTNode<T>* ptr, T val){
			while(ptr->left){
				ptr=ptr->left;
			}
		...
		void duplicati_insert(T val){
			if(isEmpty()) return;

			BSTNode<T>* ptr=search(val);
			if(ptr && duplicati){
				insert_sinistra(ptr, val);
				duplicati=false;
			}else if(ptr && !duplicati){
				insert_destra(ptr, val);
				duplicati=true;
			}
		}
