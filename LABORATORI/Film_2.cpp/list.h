...
class List {
	...
	void nuovo(){
		...
		Node<T>* ptr=head;
		int max=head->getVal().getAnno();
		Film max_f=head->getVal();
		while(ptr){
			if(ptr->getVal().getAnno() > max){
				max=ptr->getVal().getAnno();
				max_f=ptr->getVal();
			}
			ptr=ptr->getNext();
		}
		cout << "Il film piu nuovo è = " << max_f << endl;
	}
	...
	void vecchio(){
		...
		Node<T>* ptr=head;
		int min=head->getVal().getAnno();
		Film min_f=head->getVal();
		while(ptr){
			if(ptr->getVal().getAnno() < min){
				min=ptr->getVal().getAnno();
				min_f=ptr->getVal();
			}
			ptr=ptr->getNext();
		}
		cout << "Il film piu vecchio è = " << min_f << endl;
	}

	void estrazione(string s){
		if(isEmpty()) return;

		Node<T>* ptr=head;
		while(ptr){
			if(ptr->getVal().getTipo() == s){
				cout << *ptr << endl;
			}
			ptr=ptr->getNext();
		}
